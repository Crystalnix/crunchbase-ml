...
df = pd.read_csv('../data/invested_companies_and_degrees_with_offices.csv')
codes = ['b', 'angel', 'a', 'seed', 'c', 'd', 'unattributed', 'debt_round', 'e', 'f', 'private_equity', 'grant',
         'post_ipo_equity', 'post_ipo_debt', 'partial', 'convertible', 'crowd', 'g', 'secondary_market', 'crowd_equity']
suffixes = ['funded_at', 'raised_amount_usd', 'pre_money_valuation_usd', 'post_money_valuation_usd', 'participants']
df['average_funded'] = df[['%s_raised_amount_usd' % code for code in codes]].mean(axis=1)
...
raw_data = pd.read_csv('../data/data.csv')[['company_id', 'founded_at']]
...
df.loc[is_not_acquired, 'age'] = (
    ...
    .apply(lambda x: x.days)
ipo_columns = ['valuation_amount', 'valuation_currency_code', 'public_at', 'raised_amount', 'raised_currency_code',
               'stock_symbol']
