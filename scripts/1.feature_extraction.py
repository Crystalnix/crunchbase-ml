...
companies = pd.read_sql(
    """select id as company_id, name, category_code, status, founded_at, closed_at, country_code, state_code, city, region  
       from cb_objects 
       where entity_type='Company';""", con=db)
acquisitions = pd.read_sql("select acquired_object_id as company_id,  acquired_at from cb_acquisitions;", con=db)
ipos = pd.read_sql(
    "select ipo_id, object_id as company_id, valuation_amount, valuation_currency_code, raised_amount, raised_currency_code, public_at, stock_symbol from cb_ipos;",
    con=db)
...
funding_rounds = pd.read_sql(
    "select funding_round_id, object_id as company_id, funded_at, funding_round_type, funding_round_code, raised_amount_usd, pre_money_valuation_usd, post_money_valuation_usd, participants from cb_funding_rounds;",
    con=db)
...
funding_round_columns = [column for column in funding_rounds.columns if
                         column not in ['funding_round_id', 'company_id', 'funding_round_code', 'funding_round_type']]
